...
def check_seasons(base_url, name, language):
    ...
    while consecutive_not_found < 3:
        ...
        # Vérifier qu'il y a réellement des épisodes
        if has_normal:
            sibnet, vidmoly = extract_video_links(normal_url)
            has_normal = len(sibnet) > 0 or len(vidmoly) > 0
        ...
        # Vérifier qu'il y a réellement des épisodes
        if has_hs:
            sibnet, vidmoly = extract_video_links(hs_url)
            has_hs = len(sibnet) > 0 or len(vidmoly) > 0
            ...
            for base_key, base_url_var in [(f"{season}", normal_url if has_normal else None), (f"{season}hs", hs_url if has_hs else None)]:
                ...
                while variant_not_found < 3:
                    ...
                    if r.status_code == 200 and r.text.strip():
                        # Vérifier qu'il y a réellement des épisodes
                        sibnet, vidmoly = extract_video_links(variant_url)
                        if len(sibnet) > 0 or len(vidmoly) > 0:
    ...
    # Film & OAV (avec vérification des épisodes)
